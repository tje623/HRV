...
SEGMENT_DURATION_NS: int = 60_000_000_000  # 60 s
...
def _parse_timestamp_ns(val: object) -> int | None:
    """Convert epoch-ms int, ISO string, or numeric string → epoch nanoseconds."""
    if val is None:
        return None
    if isinstance(val, (int, np.integer)):
        return int(val) * MS_TO_NS
    ...
    if isinstance(val, str):
        try:
            ...
            try:
                return int(float(val)) * MS_TO_NS
            except Exception:
                return None
    return None


def _get_recording_start_ns(peaks_parquet: Path) -> int:
    if not peaks_parquet.exists():
        ...
        sys.exit(1)
    df = pd.read_parquet(peaks_parquet, columns=["timestamp_ns"])
    val = int(df["timestamp_ns"].min())
    logger.info("Recording start: %s", pd.Timestamp(val, unit="ns"))
    return val
...
class _SegmentIntervalLookup:
    ...
    def __init__(self, segment_timestamps: list) -> None:
        intervals: list[tuple[int, int, int]] = []  # (first_ns, last_ns, ann_idx)
        for entry in segment_timestamps:
            ann_idx = entry.get("segment_idx")
            ...
            if ann_idx is None or ft_str is None:
                continue
            try:
                ft_ns = int(pd.Timestamp(ft_str).value)
                lt_ns = (
                    int(pd.Timestamp(lt_str).value)
                    if lt_str
                    else ft_ns + SEGMENT_DURATION_NS
                )
                intervals.append((ft_ns, lt_ns, int(ann_idx)))
            except Exception:
                pass
        intervals.sort(key=lambda x: x[0])
        self._first_ns = [iv[0] for iv in intervals]
        self._last_ns  = [iv[1] for iv in intervals]
        self._ann_idx  = [iv[2] for iv in intervals]

    def lookup(self, ts_ns: int) -> int | None:
        """Return annotation segment index for ts_ns, or None if unmatched."""
        # Candidate: the segment whose first_ns is ≤ ts_ns (search right boundary)
        pos = bisect.bisect_right(self._first_ns, ts_ns) - 1
        if pos < 0:
            return None
        ...
        for idx in (pos, pos + 1):
            if 0 <= idx < len(self._ann_idx):
                if self._first_ns[idx] <= ts_ns <= self._last_ns[idx]:
...
def main() -> None:
    ...
    recording_start_ns = _get_recording_start_ns(args.peaks_parquet)
    ...
    ann_to_pipeline = _build_ann_to_pipeline(segment_timestamps, recording_start_ns)
    ...
    for region in all_bad_regions_raw:
        ...
        try:
            ...
            n_bad_region_skipped += 1
            continue
        bad_region_intervals.setdefault(pipeline_idx, []).append((start_ns, end_ns))
    ...
    def _in_bad_region_by_ts(ts_ns: int) -> bool:
        """Check whether ts_ns falls inside any bad_region window."""
        # Derive pipeline seg from timestamp to look up intervals
        pipeline_seg = (ts_ns - recording_start_ns) // SEGMENT_DURATION_NS
    ...
    def _drop_reason(
        ...
        ann_seg = seg_lookup.lookup(ts_ns)
        ...
        if ann_seg in revisit:
            return "in_revisit"
        if check_bad_region and _in_bad_region_by_ts(ts_ns):
    ...
    for entry in artifacts_raw:
        ts_ns = _parse_timestamp_ns(entry)
        if ts_ns is None:
            drop_art["not_validated"] += 1
            continue
        reason = _drop_reason(ts_ns, check_bad_region=True)
        if reason == "keep":
    ...
    for entry in added_raw:
        ...
        if ts_ns is None:
            ...
            continue
        reason = _drop_reason(ts_ns, check_bad_region=False)
        if reason == "keep":
    ...
    for entry in physio_raw:
        if isinstance(entry, dict):
            ...
            ts_ns = _parse_timestamp_ns(ts_val)
        else:
            ts_ns = _parse_timestamp_ns(entry)
        if ts_ns is None:
