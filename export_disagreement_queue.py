...
def main() -> None:
    ...
    # ── 3. Attach timestamp_ns + segment_idx from peaks.parquet ──────────────
    log.info("Loading peaks …")
    peaks = pd.read_parquet(args.peaks, columns=["peak_id", "timestamp_ns", "segment_idx"])
    ...
    cand["timestamp_ns"] = cand["timestamp_ns"].fillna(0).astype("int64")
    ...
    for seg_num, (seg_idx, group) in enumerate(seg_groups, 1):
        ...
        try:
            ...
            ecg_chunk = tbl.to_pandas().sort_values("timestamp_ns").reset_index(drop=True)
        ...
        ecg_ts   = ecg_chunk["timestamp_ns"].values
        ...
        for _, row in group.iterrows():
            ...
            if mask.any():
                ...
                closest = int(np.argmin(np.abs(ecg_ts[mask] - ts_ns)))
