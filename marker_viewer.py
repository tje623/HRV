...
# ── Constants ─────────────────────────────────────────────────────────────────
MARKER_CSV   = Path("/Volumes/xHRV/Accessory/Marker.csv")
SAMPLE_RATE  = 130
DISPLAY_NS   = int(60 * 1e9)
MIN_VIEW_NS  = int(10 * 1e9)
MAX_VIEW_NS  = int(120 * 1e9)
ZOOM_FACTOR  = 1.15
RR_SHOW_NS   = int(30 * 1e9)
LOCAL_TZ     = ZoneInfo("America/New_York")

_early_cutoff_dt = datetime(2025, 8, 1, tzinfo=LOCAL_TZ)
EARLY_CUTOFF_NS  = int(_early_cutoff_dt.astimezone(timezone.utc).timestamp() * 1e9)
SEARCH_STEPS_RECENT = [int(5  * 60 * 1e9)]
SEARCH_STEPS_EARLY  = [int(5  * 60 * 1e9),
                        int(10 * 60 * 1e9),
                        int(15 * 60 * 1e9)]
...
def _to_ns(dt_local: datetime) -> int:
    return int(dt_local.astimezone(timezone.utc).timestamp() * 1e9)
...
def _load_markers() -> list[dict]:
    ...
    for raw in df[col]:
        try:
            ...
            out.append({"dt": dt, "ns": _to_ns(dt), "raw": str(raw).strip()})
        except ValueError:
            continue
    return out
...
def _load_segments(processed: Path) -> pd.DataFrame:
    return pq.read_table(
        ...
        columns=["segment_idx", "start_timestamp_ns", "end_timestamp_ns"],
    ).to_pandas()


def _find_segment(marker_ns: int, segments: pd.DataFrame,
                  search_steps: list[int]) -> tuple[int | None, int]:
    for radius_ns in search_steps:
        lo, hi = marker_ns - radius_ns, marker_ns + radius_ns
        hits = segments[
            (segments["end_timestamp_ns"] >= lo) &
            (segments["start_timestamp_ns"] <= hi)
        ].copy()
        if len(hits) == 0:
            continue
        hits["mid_ns"] = (hits["start_timestamp_ns"] + hits["end_timestamp_ns"]) // 2
        hits["dist"]   = (hits["mid_ns"] - marker_ns).abs()
        best = hits.loc[hits["dist"].idxmin()]
        return int(best["segment_idx"]), int(best["dist"] / 1e9)
    return None, -1


def _load_ecg(processed: Path, seg_idx: int,
              lo_ns: int, hi_ns: int) -> tuple[np.ndarray, np.ndarray]:
    tbl = pq.read_table(
    ...
    ).to_pandas().sort_values("timestamp_ns")
    return (tbl["timestamp_ns"].values.astype(np.int64),
            tbl["ecg"].values.astype(np.float32))


def _load_peaks(processed: Path, lo_ns: int, hi_ns: int) -> pd.DataFrame:
    tbl = pq.read_table(
    ...
    tbl["timestamp_ns"] = tbl["timestamp_ns"].astype(np.int64)
    return tbl.sort_values("timestamp_ns").reset_index(drop=True)
...
class MarkerAnnotator:
    ...
    def __init__(self, processed: Path, start_idx: int = 0,
        ...
        self.view_width_ns     : int             = DISPLAY_NS
    ...
    def _build_annotation_windows(self) -> list[int]:
        ...
        for c in centers[1:]:
            if c - clusters[-1][-1] <= _ANN_MERGE_NS:
                clusters[-1].append(c)
            else:
                clusters.append([c])
    ...
    def _go_to_annotation_window(self, idx: int) -> None:
        ...
        self.view_width_ns   = int(60e9)    # ±30 s flanks
        self.state           = "BROWSE"
        self.selected_peaks  = []
        steps = (SEARCH_STEPS_EARLY if center_ns < EARLY_CUTOFF_NS
    ...
    def _go_to_marker(self, idx: int) -> None:
        ...
        self.marker_idx     = idx
        m                   = self.markers[idx]
        steps               = (SEARCH_STEPS_EARLY if m["ns"] < EARLY_CUTOFF_NS
                                else SEARCH_STEPS_RECENT)
    ...
    def _load_window(self) -> None:
        ...
        if self.seg_idx is None:
            ...
            self.peaks_df   = pd.DataFrame(columns=["peak_id", "timestamp_ns"])
            self.ecg_loaded = False
            return

        overlapping = self.segments[
            (self.segments["end_timestamp_ns"] >= lo) &
            (self.segments["start_timestamp_ns"] <= hi)
        ]
        ts_parts, ecg_parts = [], []
    ...
    def _draw(self) -> None:
        ...
        if self.ecg_loaded:
            ...
            if self.selected_peaks and len(self.peaks_df):
                ...
                if len(sel):
                    sel_ns = sel["timestamp_ns"].values.astype(np.int64)
        ...
        view_sec = self.view_width_ns / 1e9

        # Marker star reference line
        if self.markers:
            m          = self.markers[self.marker_idx]
            marker_off = (m["ns"] - lo_ns) / 1e9
        ...
        if self.state == "SEG_END" and self._seg_start_ns is not None:
            s_off = (self._seg_start_ns - lo_ns) / 1e9
            if 0 <= s_off <= view_sec:
        ...
        if (self.ecg_loaded and len(self.peaks_df) > 1
                and self.view_width_ns <= RR_SHOW_NS):
            ylim = self.ax.get_ylim()
            y_rr = ylim[0] + 0.97 * (ylim[1] - ylim[0])
            sorted_ts = np.sort(
                self.peaks_df["timestamp_ns"].values.astype(np.int64)
            )
            in_view = (sorted_ts >= lo_ns) & (sorted_ts <= hi_ns)
            view_ts = sorted_ts[in_view]
            for i in range(len(view_ts) - 1):
                rr_ms = int(round((view_ts[i + 1] - view_ts[i]) / 1e6))
                mid_t = ((view_ts[i] + view_ts[i + 1]) / 2 - lo_ns) / 1e9
        ...
        tick_ns   = [lo_ns + int(s * 1e9) for s in tick_sec]
        self.ax.set_xticks(tick_sec)
        self.ax.set_xticklabels(
            [datetime.fromtimestamp(ns / 1e9, tz=LOCAL_TZ).strftime("%H:%M:%S")
             for ns in tick_ns],
        ...
        if self.annotations_only:
            dt_str = datetime.fromtimestamp(
                self.view_center_ns / 1e9, tz=LOCAL_TZ
    ...
    def _draw_existing_annotations(
        ...
        for ann in self.seg_anns:
            ...
            if ann_lo > hi_ns or ann_hi < lo_ns:
                continue
            x0  = max(0, (ann_lo - lo_ns) / 1e9)
            x1  = min(self.view_width_ns / 1e9, (ann_hi - lo_ns) / 1e9)
    ...
    def _on_scroll(self, event) -> None:
        ...
        new_width = max(MIN_VIEW_NS, min(MAX_VIEW_NS, new_width))
    ...
    def _on_click(self, event) -> None:
        ...
        lo_ns    = self.view_center_ns - self.view_width_ns // 2
        click_ns = lo_ns + int(event.xdata * 1e9)

        if self.state == "BROWSE":
            if len(self.peaks_df) == 0:
                return
            dists     = (self.peaks_df["timestamp_ns"] - click_ns).abs()
            nearest_i = dists.idxmin()
            if dists[nearest_i] > SNAP_THRESHOLD_NS:
                return
            ...
            best_i, best_dist = None, SNAP_THRESHOLD_NS
    ...
    def _save_segment_annotation(self, theme_id: int) -> None:
        ...
        with open(sp, "a", newline="") as f:
            ...
            if write_header:
                w.writeheader()
            w.writerow(row)
        dur_ms = (self._seg_end_ns - self._seg_start_ns) // 1_000_000
    ...
    def _save_beat_annotations(self, theme_id: int) -> None:
        ...
        with open(bp, "a", newline="") as f:
            ...
            for pid in self.selected_peaks:
                ...
                if len(prow) == 0:
                    continue
                pts_ns = int(prow.iloc[0]["timestamp_ns"])
                row = {
                    "ann_id":            self._next_ann_id,
                    "marker_idx":        midx,
                    "marker_dt":         mdt,
    ...
    def _export_screenshots(self) -> None:
        ...
        for key in all_keys:
            ...
            pk_wide = _load_peaks(self.processed,
                                  ...
                                  max(ts_pts) + int(8e9))
            if len(pk_wide) >= 2:
                sorted_ts = np.sort(pk_wide["timestamp_ns"].values.astype(np.int64))
                ...
                win_lo = min(ts_pts) - int(5e9)
                win_hi = max(ts_pts) + int(5e9)

            overlapping = self.segments[
                (self.segments["end_timestamp_ns"] >= win_lo) &
                (self.segments["start_timestamp_ns"] <= win_hi)
            ]
            ...
            if len(pk_win):
                sn_all, sy_all = _snap_peaks(
                    pk_win["timestamp_ns"].values.astype(np.int64),
                    ecg_ts, ecg_vals,
                )
            else:
            ...
            for ann in b_anns:
                ...
                ax.scatter([(sn[0] - win_lo) / 1e9], [float(sy[0])],
                           color=c, s=100, zorder=7,
                           marker="D", edgecolors="white", linewidths=0.8)

            view_sec = (win_hi - win_lo) / 1e9
            for ann in s_anns:
                x0 = (int(ann["view_start_ns"]) - win_lo) / 1e9
                x1 = (int(ann["view_end_ns"])   - win_lo) / 1e9
            ...
            ax.set_xticks(tk)
            ax.set_xticklabels(
                [datetime.fromtimestamp((win_lo + s * 1e9) / 1e9,
