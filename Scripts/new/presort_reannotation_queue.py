...
Window extraction strategy:
  ...
  • Fallback: for the rare beats with empty context_ecg (context_len == 0),
    load the 64-sample window from ecg_samples.parquet using timestamp_ms.
...
def _extract_window_from_parquet(
    ...
    sample to timestamp_ms via binary search, then applies snap + extraction.

    Args:
        timestamp_ms: R-peak timestamp in milliseconds.
    ...
    insert_idx = int(np.searchsorted(ecg_ts, timestamp_ms, side="left"))
    center = insert_idx
    if 0 < center < n_ecg:
        if abs(ecg_ts[center - 1] - timestamp_ms) < abs(ecg_ts[center] - timestamp_ms):
            center -= 1
    elif center >= n_ecg:
        center = n_ecg - 1
...
def presort_queue(
    ...
    for i, json_path in enumerate(json_files):
        ...
        ts_ms     = int(beat.get("timestamp_ms", 0))
    ...
    if fallback_needed:
        ...
        for rec_idx in fallback_needed:
            ...
            windows_list[rec_idx] = _extract_window_from_parquet(
                ts_ms, seg_idx, ecg_samples_path
            )
            logger.debug(
