...
from config import PROCESSED_DIR, SAMPLE_RATE_HZ
...
def _load_beats(sample_df: pd.DataFrame) -> list[dict]:
    ...
    context_ms = int(CONTEXT_SEC * 1000)
    ...
    for _, row in df.iterrows():
        ...
        mask = (
            (seg_ecg["timestamp_ms"] >= ts - context_ms)
            & (seg_ecg["timestamp_ms"] <= ts + context_ms)
        )
        ctx = seg_ecg[mask]
        if len(ctx) < 10:
        ...
        ctx_ts  = ctx["timestamp_ms"].values.astype(np.int64)
...
def main() -> None:
    ...
    ens = pd.read_parquet(FULL / "ensemble_preds.parquet")  # FULL = PROCESSED_DIR
