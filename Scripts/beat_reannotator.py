...
def apply_presort_order(beats: list[dict], presorted_path: Path) -> list[dict]:
    ...
    The presort CSV (produced by presort_reannotation_queue.py) has columns:
        peak_id, queue, timestamp_ms, global_corr_clean, suggested_category
...
class _State:
    ...
    def _append_csv(self, idx: int, tags: list[dict]) -> None:
        ...
        with open(self.output_path, "a", newline="") as f:
            w = csv.writer(f)
            if write_header:
                w.writerow(["queue", "peak_id", "timestamp_ms",
                             "old_label", "tags", "annotated_at"])
            w.writerow([
                b["queue"], b["peak_id"], b["timestamp_ms"],
...
function renderECG(beat) {{
  ...
  let t;
  if (beat.context_ts_ms && beat.context_ts_ms.length === ecg.length) {{
    const t0 = beat.context_ts_ms[0];
    t = beat.context_ts_ms.map(v => (v - t0) / 1000);
  }} else {{
