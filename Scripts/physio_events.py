...
def load_peaks(peaks_path: Path) -> pd.DataFrame:
    ...
    peaks = pq.read_table(
        peaks_path, columns=["peak_id", "timestamp_ms", "segment_idx"]
    ).to_pandas()
    logger.info("  %d peaks loaded", len(peaks))

    peaks = peaks.sort_values("timestamp_ms").reset_index(drop=True)
    peaks["rr_ms"] = peaks["timestamp_ms"].diff()
...
def detect_vagal_arrests(
    ...
    timestamps = peaks["timestamp_ms"].values
    ...
    for i in candidate_idxs:
        ...
        arrests.append({
            ...
            "onset_timestamp_ms":  ts,
            "onset_time":          pd.Timestamp(ts, unit="ms").isoformat(),
            "rr_ms":               float(rr[i]),
    ...
    df = pd.DataFrame(arrests) if arrests else pd.DataFrame(
        columns=["onset_peak_id", "onset_timestamp_ms", "onset_time",
    ...
    for sess_start, sess_end in sessions:
        ...
        def _flush(ex: list) -> None:
            ...
            all_events.append({
                ...
                "start_time":             pd.Timestamp(ts_s, unit="ms").isoformat(),
                "end_time":               pd.Timestamp(ts_e, unit="ms").isoformat(),
                "duration_s":             (ts_e - ts_s) / 1000,
...
def main() -> None:
    ...
    p.add_argument("--peaks", type=Path,
                   default=PEAKS_PARQUET,
                   help=f"Path to peaks.parquet (default: {PEAKS_PARQUET})")
    p.add_argument("--output-dir", type=Path,
                   default=AUTODETECTED_DIR,
                   help=f"Directory for output CSVs (default: {AUTODETECTED_DIR})")
