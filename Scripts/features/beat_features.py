...
def _compute_extended_rr(
    ...
    Args:
        timestamps_ms: Sorted int64 peak timestamps in milliseconds.
    ...
    rr_all = np.diff(timestamps_ms.astype(np.float64))  # (n-1,) already in ms
...
def compute_beat_feature_matrix(
    ...
    timestamps = peaks_sorted["timestamp_ms"].values.astype(np.int64)
    ...
    if n > 1:
        diffs_ms = np.diff(timestamps.astype(np.float64))  # already in ms
        rr_prev[1:] = diffs_ms
        rr_next[:-1] = diffs_ms
...
def _process_chunk_worker(task: dict) -> tuple[int, str]:
    ...
    # labels.parquet is written in peak (timestamp) order, so timestamp_ms
    # predicate pushdown skips unneeded row groups efficiently.
    ts_min = int(chunk_peaks["timestamp_ms"].min())
    ts_max = int(chunk_peaks["timestamp_ms"].max())
    chunk_labels_raw = (
        pq.read_table(
            labels_path,
            filters=[
