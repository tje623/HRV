...
def _impute(df: pd.DataFrame, medians: dict[str, float], cols: list[str]) -> pd.DataFrame:
    ...
    for c in cols:
        if c in df.columns and df[c].isna().any():
            ...
            imputed_cols.append(f"{c}({n_nan}→{fill:.4g})")
    if imputed_cols:
        logger.debug("Imputed NaN values: %s", ", ".join(imputed_cols))
    return df
...
def _assign_category(
    ...
    total = n_clean + n_art
    if total == 0:
        logger.debug("_assign_category leaf_id=%d: no samples → 'uncertain'", leaf_id)
        return "uncertain"
    clean_frac = n_clean / total
    art_frac   = n_art   / total
    logger.debug(
        "_assign_category leaf_id=%d: n_clean=%d n_art=%d clean_frac=%.3f art_frac=%.3f",
        leaf_id, n_clean, n_art, clean_frac, art_frac,
    )
...
def fit_mode(args: argparse.Namespace) -> None:
    ...
    logger.info("=== fit_mode complete: %d leaves, %s", clf.get_n_leaves(), str(cat_train_counts))
    logger.info("Outputs: rules=%s  tree=%s  model=%s", rules_path, tree_viz_path, model_path)
    print()
    print("═" * 68)
...
def apply_mode(args: argparse.Namespace) -> None:
    ...
    logger.info("=== apply_mode complete: %d beats, category distribution: %s ===", total_rows, cat_totals)
    print()
    print("═" * 60)
...
def encode_mode(args: argparse.Namespace) -> None:
    ...
    logger.info(
        "=== encode_mode complete: %d rows × %d cols, %d category dummies: %s ===",
        len(enriched), len(enriched.columns), len(cat_cols), cat_cols,
    )
    print()
...
def main() -> None:
    ...
    global logger
    logger = setup_logger("auto_categorize_beats", args=args, disable_log=args.no_log)
    logger.info("=== auto_categorize_beats started | command=%s ===", args.command)
    logger.debug("Config: VAL_FRACTION=%.3f  LGBM_RANDOM_STATE=%d", VAL_FRACTION, LGBM_RANDOM_STATE)
    if args.command == "fit":
        fit_mode(args)
