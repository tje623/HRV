...
def export_queue(
    ...
    ecg_samples_df : pd.DataFrame
        Must have ``timestamp_ms``, ``ecg``, ``segment_idx``.
    output_path : str
    ...
    # Ensure peak_id → timestamp_ms and segment_idx
    peak_cols = ["peak_id", "timestamp_ms", "segment_idx"]
    if "timestamp_ms" not in cand.columns or "segment_idx" not in cand.columns:
        cand = cand.merge(
        ...
        for col in ["timestamp_ms", "segment_idx"]:
            alt = f"{col}_pk"
    ...
    ecg = ecg_samples_df.sort_values("timestamp_ms").reset_index(drop=True)
    ecg_ts = ecg["timestamp_ms"].values
    ...
    for _, row in cand.iterrows():
        ...
        if window_mask.any():
            ...
            if len(context_ts) > 0:
                r_peak_idx = int(np.argmin(np.abs(np.array(context_ts) - ts_ms)))
        else:
        ...
        record = {
            ...
            "context_timestamps_ms": context_ts,
