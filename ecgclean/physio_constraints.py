...
# Average HR bounds — from ~9 months of validated beat histogram
# Modal range is 90-120 bpm (~60% of all beats)
# These apply to AVERAGED heart rate, not instantaneous RR intervals
HR_MODAL_LOW: int = 90
HR_MODAL_HIGH: int = 120

# Soft-flag thresholds — beats outside these get flagged for review
# but are NEVER hard-filtered
HR_SUSPICIOUS_LOW_BPM: int = 40       # avg HR below this warrants review
HR_SUSPICIOUS_HIGH_BPM: int = 200     # avg HR above this warrants review

# Soft-flag RR thresholds — wide enough to accommodate genuine physiology
# A 3-second vagal pause is real. A 200ms RR during SVT is real.
# These are review flags only, not exclusion criteria.
RR_SUSPICIOUS_SHORT_MS: int = 220     # ms — below this, flag but never hard-filter
RR_SUSPICIOUS_LONG_MS: int = 3200     # ms — above this, flag but never hard-filter

# POTS transition dynamics — validated from direct observation
# Fastest confirmed genuine drop: ~186 -> 70 bpm in ~13.5 seconds
# Personally observed single-second drops of 20-25 bpm
# These are used for soft feature computation only
POTS_MAX_DELTA_HR_PER_SECOND: float = 25.0   # bpm/sec — observed upper bound
POTS_TRANSITION_WINDOW_SEC: int = 45          # seconds on each side of a phys_event

# Tachycardic transition detection — auto-detected from HR trajectory, no labels needed
# Uses a 10-beat rolling-mean smoothed HR (removes HRV noise) compared against the
# minimum smoothed HR seen in the past TACHY_TRANSITION_LOOK_AHEAD_SEC seconds.
# Threshold is 35 bpm: genuine POTS orthostatic surges are 40-70 bpm; 35 bpm gives
# margin while never firing on normal beat-to-beat variability (~5-25 bpm at 130 Hz).
TACHY_TRANSITION_MIN_RISE_BPM: float = 35.0  # bpm — minimum smoothed-HR rise to qualify
TACHY_TRANSITION_LOOK_AHEAD_SEC: int = 60     # seconds — backward window for smoothed floor
TACHY_TRANSITION_SMOOTH_BEATS: int = 10       # beats — rolling mean window before comparison

# Artifact pattern thresholds — these CAN be hard filters because
# no cardiac mechanism produces them
RR_IMPOSSIBLE_NEGATIVE_OR_ZERO: int = 0       # ms — data integrity failure
# NOTE: consecutive bit-identical RR detection was removed. At 130 Hz (~7.69 ms
# resolution), many genuine consecutive beats naturally quantize to the same RR
# value, so the check produces massive false-positive hard-filtering.

# ── Derived constants (internal) ───────────────────────────────────────────────

_MS_TO_NS: int = 1_000_000
_POTS_WINDOW_NS: int = POTS_TRANSITION_WINDOW_SEC * 1_000_000_000

# Sandwich detection: a spurious peak wedged between two real beats
_SANDWICH_SHORT_NS: int = 150 * _MS_TO_NS     # 150 ms — no real beat this close
_NORMAL_RR_LOW_NS: int = 300 * _MS_TO_NS      # 300 ms (~200 bpm upper normal)
_NORMAL_RR_HIGH_NS: int = 1200 * _MS_TO_NS    # 1200 ms (~50 bpm lower normal)
...
def apply_hard_filters(
    ...
    Args:
        peaks_df: Canonical peaks table with timestamp_ns and peak_id.
    ...
    peaks_sorted = peaks_df.sort_values("timestamp_ns").reset_index(drop=True)
    ts = peaks_sorted["timestamp_ns"].values.astype(np.int64)
    ...
    orig_ts = peaks_df["timestamp_ns"].values.astype(np.int64)
    if len(orig_ts) > 1:
    ...
    if n > 3:
        ...
        sandwich_mask = (
            ...
            & (rr_prev < _SANDWICH_SHORT_NS)
            & (rr_next > _SANDWICH_SHORT_NS)
            & (neighbor_prev_rr >= _NORMAL_RR_LOW_NS)
            & (neighbor_prev_rr <= _NORMAL_RR_HIGH_NS)
            & (neighbor_next_rr >= _NORMAL_RR_LOW_NS)
            & (neighbor_next_rr <= _NORMAL_RR_HIGH_NS)
            & ~hard_filtered
        )
...
def build_pots_windows(labels_df: pd.DataFrame) -> list[tuple[int, int]]:
    ...
    Requires labels_df to contain 'timestamp_ns' and 'phys_event_window'
    columns.  If 'timestamp_ns' is missing, logs a warning and returns
    an empty list.

    Args:
        labels_df: Labels table enriched with timestamp_ns.

    Returns:
        Sorted list of (start_ns, end_ns) tuples representing POTS
        transition windows (with buffer applied and overlaps merged).
    """
    if "timestamp_ns" not in labels_df.columns:
        logger.warning(
            "labels_df missing 'timestamp_ns' column; cannot build POTS windows"
        )
        return []
    ...
    df = labels_df.sort_values("timestamp_ns").reset_index(drop=True)
    ...
    for block in blocks:
        if len(block) == 0:
            continue
        block_start_ns = int(ts[block[0]])
        block_end_ns = int(ts[block[-1]])
        # Extend by POTS_TRANSITION_WINDOW_SEC on each side
        extended_start = block_start_ns - _POTS_WINDOW_NS
        extended_end = block_end_ns + _POTS_WINDOW_NS
...
def detect_tachycardic_transitions(ts_ns: np.ndarray, hr_bpm: np.ndarray) -> np.ndarray:
    ...
    Args:
        ts_ns: Sorted int64 timestamps in nanoseconds (length n).
    ...
    # ts_ns must be monotonically increasing (caller guarantees sort-by-timestamp).
    dt_index = pd.to_datetime(ts_ns, unit="ns")
    ...
    for block in blocks:
        if len(block) == 0:
            continue
        block_start_ns = int(ts_ns[block[0]])
        block_end_ns = int(ts_ns[block[-1]])
        raw_windows.append(
            (block_start_ns - _POTS_WINDOW_NS, block_end_ns + _POTS_WINDOW_NS)
        )
    ...
    return _is_in_any_window(ts_ns, merged)
...
def compute_soft_features(
    ...
    All features are computed vectorially in nanoseconds internally, then
    converted to ms/bpm for output columns.

    Args:
        peaks_df: Canonical peaks table (timestamp_ns, peak_id).
    ...
    if "timestamp_ns" not in labels_df.columns:
        labels_df = labels_df.merge(
            peaks_df[["peak_id", "timestamp_ns"]], on="peak_id", how="left"
        )
    ...
    # ── RR intervals ──────────────────────────────────────────────────────
    rr_prev_ns = np.full(n, np.nan)
    rr_next_ns = np.full(n, np.nan)
    if n > 1:
        diffs = np.diff(ts).astype(np.float64)
        rr_prev_ns[1:] = diffs
        rr_next_ns[:-1] = diffs

    rr_prev_ms = rr_prev_ns / _MS_TO_NS
    rr_next_ms = rr_next_ns / _MS_TO_NS
    ...
    df["in_modal_hr_range"] = (
        hr_valid & (hr_bpm >= HR_MODAL_LOW) & (hr_bpm <= HR_MODAL_HIGH)
    )
    ...
    tachy_transition = detect_tachycardic_transitions(ts, hr_bpm)
    ...
    df["physio_implausible"] = (
        rate_valid
        & not_hard
        & (hr_change_rate > POTS_MAX_DELTA_HR_PER_SECOND)
        & ~pots_candidate
        & ~tachy_transition
...
def _is_in_any_window(
    ...
    Args:
        timestamps: int64 array of beat timestamps in nanoseconds.
        windows: List of (start_ns, end_ns) tuples.
    ...
    for start_ns, end_ns in windows:
        result |= (timestamps >= start_ns) & (timestamps <= end_ns)
    return result
