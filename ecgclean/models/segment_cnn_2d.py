...
IMAGE_SIZE = (64, 64)
N_SCALES = 64
MIN_SAMPLES = 512  # Minimum ECG samples to compute a meaningful scalogram
...
def compute_scalogram(
    ...
    sampling_rate : int
        Sample rate in Hz (default 256 for Polar H10).
    ...
    np.ndarray
        ...
        Returns a zero array if the segment has < 512 samples.
    """
    if len(ecg_segment) < MIN_SAMPLES:
        log.warning(
            "Segment has only %d samples (< %d) — returning zero scalogram",
            len(ecg_segment),
            MIN_SAMPLES,
        )
...
def _load_or_compute_scalogram(
    ...
    seg_ecg = ecg_samples_df.loc[mask].sort_values("timestamp_ns")["ecg"].values
...
class SegmentScalogramDataset(Dataset):
    ...
    ----------
    ecg_samples_df : pd.DataFrame
        ECG samples with ``timestamp_ns``, ``ecg``, ``segment_idx``.
