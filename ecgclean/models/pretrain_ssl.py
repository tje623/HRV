...
def _extract_windows(
    ...
    sample_interval_ns = int(1e9 / SAMPLE_RATE)  # 8 000 000 ns (exactly 8 ms at 125 Hz)
    ...
    for i in range(n_beats):
        ...
        target_ts = (
            t_start + np.arange(WINDOW_SIZE, dtype=np.int64) * sample_interval_ns
        )
...
def _prepare_windows(
    ...
    if len(peaks_df) == 0:
        ...
        return np.zeros((0, WINDOW_SIZE), dtype=np.float32), np.array([], dtype=np.int64)

    # Extract windows
    windows = _extract_windows(
        peak_timestamps_ns=peaks_df["timestamp_ns"].values,
...
def main() -> None:
    ...
    p_pretrain.add_argument(
        "--max-epochs", type=int, default=50, help="Maximum training epochs"
    )
    p_pretrain.add_argument(
